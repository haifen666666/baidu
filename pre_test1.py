...
def get_visit_hour():
    ...
    for file in file_lis:
        ...
        cur_num += 1
    visit = pd.DataFrame(visit,columns = ['year','month','day','hour','numbers','area_id'])
    visit.to_csv('visit_hour_test.csv',index=False)
    end = time.time()
    print(end-start)
...
def get_visit_quarter():
    ...
    visit = pd.DataFrame(visit,columns = ['year','month','day','state','numbers','area_id'])
    visit.to_csv('visit_quarter_test.csv',index=False)
    end = time.time()
    print(end-start)
